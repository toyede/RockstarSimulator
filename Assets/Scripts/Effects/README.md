...
- 앙코르·무대 장악 카드는 카드 이름만 표시하며, 등급은 `CardUpgradeVFX`의 순환 테두리 색으로 표시한다.
- `CardSlotUI`는 현재 런 덱의 카드 ID와 `sourceAugmentId`/`sourceTier`로 획득 여부와 등급을 확인한다. 현재 등급별 카드는 서로 다른 ID를 사용한다.
- VFX는 카드 그림의 실제 표시 영역에 맞추고, 사용 시 짧게 발광한 뒤 종료한다. 풀 재사용 시 색상·발광 상태를 초기화한다.
- 드로우·리롤 강화는 기존 `CardUpgradeFrame` 아트를 사용한다. 증강 선택·보유 화면의 등급 표기는 유지한다.
